describe EmsEvent do
  ...
  context "container events" do
    ...
    context "on node" do
      ...
      it "process_container_entities_in_event! should link node id to event" do
        EmsEvent.process_container_entities_in_event!(@event_hash)
        expect(@event_hash[:container_node_id]).to eq @container_node.id
      end

      it "src_container_node should find it" do
        event = EmsEvent.add(@ems.id, @event_hash)
        expect(event.send(:src_container_node)).to eq @container_node
      end
    end
    ...
    context "on pod" do
      ...
      it "process_container_entities_in_event! should link pod id to event" do
        EmsEvent.process_container_entities_in_event!(@event_hash)
        expect(@event_hash[:container_group_id]).to eq @container_group.id
      end

      it "src_container_group should find it" do
        event = EmsEvent.add(@ems.id, @event_hash)
        expect(event.send(:src_container_group)).to eq @container_group
      end
    end
    ...
    context "on replicator" do
      ...
      it "process_container_entities_in_event! should link replicator id to event" do
        EmsEvent.process_container_entities_in_event!(@event_hash)
        expect(@event_hash[:container_replicator_id]).to eq @container_replicator.id
      end

      it "src_container_replicator should find it" do
        event = EmsEvent.add(@ems.id, @event_hash)
        expect(event.send(:src_container_replicator)).to eq @container_replicator
      end
    end
  end
